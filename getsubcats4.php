...
$S3 = isset($_REQUEST['subcat3']) ? $_REQUEST['subcat3'] : ""; // name (not #)
...
setcookie("last_subcat2",$S2,time()+86400,"/",COOKIEDOMAIN);
setcookie("last_subcat3",$S3,time()+86400,"/",COOKIEDOMAIN);

$PIDX = getPrimaryIDX($P);
$SUB1IDX = getSubcat1IDX($PIDX,$S);
$SUB2IDX = getSubcat2IDX($PIDX,$SUB1IDX,$S2);
$SUB3IDX = getSubcat3IDX($PIDX,$SUB1IDX,$SUB2IDX,$S3);

$SUBCATS4 = getSubcat4($PIDX, $SUB1IDX, $SUB2IDX, $SUB3IDX);

$SUBS = "";

if(!empty($SUBCATS4)){
	foreach($SUBCATS4 AS $SUBC){
		if($SUBC != ""){
