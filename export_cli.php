...
$skip_post = isset($_REQUEST['skippost']) ? $_REQUEST['skippost'] : 1;
...
$export_file = $edit . "/" . DEFAULTYARD . "-$dt" . "-MAG-MPOST.csv";
$export_file_qb = $edit . "/" . DEFAULTYARD . "-$dt" . "-QB-MPOST.csv";
...
$rows = mysqli_num_rows($rs);
 ...
 if($TYPE == "qbmagento"){
	if($rows >0){
		...
		fputs($fp,"_attribute_set,Item Name,Purchase Description,Lot #,Pallet #,width,length,height,weight,sqft,yard,zone,bin_location,Sales Description,grade,Color,Cost (Without freight),Cost,Price,Small Image,Thumbnail,Categories Path,Quantity,COGS Account,Income Account,Asset Account\n");
		...
		//	$line .= '"' . str_replace("/images/catalog","", str_replace("$HDIR","", $i1)) . "\","; //top view
			...
			$line .= '"' . str_replace("/images/catalog","",str_replace("$HDIR","", $i2)) . "\",";
			$line .= '"' . $STORECATEGORY . "|" . str_replace("/","-",stripslashes($ITEM['category']));// . "\",";
		...
		//--------------------- QB ES Specific
			...
			$qbline .= '"' . str_replace("/images/catalog","",str_replace("$HDIR","", $i2)) . "\",";
//			$qbline .= '"' . str_replace("/images/catalog","",str_replace("$HDIR","", $i2)) . "\",";
			//$qbline .= '"' . $STORECATEGORY . "|" . str_replace("/","-",stripslashes($ITEM['category'])) . "\",";
			$qbline .= "\"$pri\",";
